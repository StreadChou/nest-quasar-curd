...
import {FindAllBody, FindAllResponse, FindOneQuery, FindWhereBrackets, FindWhereInterface, FindWhereType, SearchBody} from './CurdBaseDefine'
export type PrimaryKeyType = string | number

export abstract class CurdBaseServices<T extends ObjectLiteral = any> {
    ...
    async CURD_FindAll(body: FindAllBody): Promise<FindAllResponse<T>> {
        ...
        CurdBaseServices.QbSearch(qb, body, this.primaryKey.toString());
    ...
    async CURD_FindOne(id: PrimaryKeyType, opt?: FindOneQuery): Promise<T | null> {
        opt = opt || {}
        const qb = this._repository.createQueryBuilder('entity');
        CurdBaseServices.QbAddSelect(qb, opt.select);
        CurdBaseServices.QbAddRelations(qb, opt.relations, opt.relationSelect);
    ...
    static QbSearch(qb: SelectQueryBuilder<any>, opt: SearchBody<any>, defaultKey: string) {
        CurdBaseServices.QbAddSelect(qb, opt.select);
        CurdBaseServices.QbAddOrder(qb, opt.orderBy, defaultKey);
        CurdBaseServices.QbAddRelations(qb, opt.relations, opt.relationSelect);
        CurdBaseServices.HandleQueryBuilderWhere(qb, opt);
    }
