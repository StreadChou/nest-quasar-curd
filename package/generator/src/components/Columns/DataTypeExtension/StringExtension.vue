...
import {WhenColumnIsIntegerTypeList, WhenColumnIsStringTypeList} from "app/src-ssr/types/SimpleColumnType";
...
<template>
  <div class="q-py-sm" style="border: dashed 1px gray">
    <div class="q-gutter-y-sm q-px-sm">
      <div class="q-pl-xs text-grey-9 q-mb-xs">String扩展编辑</div>
      <q-select v-model="model.dbType" :options="WhenColumnIsStringTypeList"
                label="数据类型" dense standout
                clearable
      />
      <q-input v-model.number="model.length" clearable standout type="number" dense label="长度"/>
      <q-input v-model="model.default" clearable standout type="text" dense label="默认值"/>
    </div>
  </div>
